...
function dayInfo(dateISO, state, program) {
  ...
  const skipped = state.extraRests.filter((d) => d >= state.startDate && d < dateISO).length;
  const idx = offset - skipped;
...
export default function App() {
  const [state, setState] = useState(() => loadState() || { startDate: todayISO(), extraRests: [], completed: [] });
  ...
  const pct = TOTAL_WORKOUTS > 0 ? Math.min(100, Math.round((completedCount / TOTAL_WORKOUTS) * 100)) : 0;
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 sm:py-6 space-y-4">
        ...
        <section ref={cardRef} className="scroll-mt-4" key={selectedDate}>
          <HeroCard
            ...
            onComplete={() => toggleSet('completed', selectedDate)}
            onRest={() => toggleSet('extraRests', selectedDate)}
          />
        </section>
      ...
      <SettingsDialog
        ...
        onReset={() => {
          if (confirm('Reset all progress?')) {
            const fresh = { startDate: todayISO(), extraRests: [], completed: [] };
            setState(fresh);
...
function TopBar({ config, onSettings, completed, total, pct }) {
  return (
...
function HeroCard({ dateISO, info, state, program, isToday, onBackToToday, onComplete, onRest }) {
  const date = parseISO(dateISO);
  const dateLabel = date.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' });
  const isDone = state.completed.includes(dateISO);
  const isExtraRest = state.extraRests.includes(dateISO);
  const canDo = info.kind === 'workout' || info.kind === 'rest';
    ...
    <article className="bg-surface-2 rounded-xl overflow-hidden fade-up">
      ...
      <div className="p-5 sm:p-6 space-y-4">
        ...
        <div className="flex items-center flex-wrap gap-2">
          {isToday ? (
            <Chip tone="primary">
              <span className="w-1.5 h-1.5 rounded-full bg-primary-on dot-pulse" />
              Today
            </Chip>
          ) : (
          ...
          {!isToday && (
            <button
              ...
              className="ml-auto text-[13px] text-primary hover:text-primary-hover px-3 py-1 rounded-full state-layer"
            >
              ← Back to today
            </button>
          )}
        ...
        {/* Title */}
        {info.kind === 'workout' && (
          <h2 className="text-[24px] sm:text-[28px] font-medium text-fg leading-tight tracking-tight">
            {info.entry.n}
          </h2>
        )}
        {info.kind === 'rest' && (
          <h2 className="text-[24px] sm:text-[28px] font-medium text-fg leading-tight">
            Rest day
          </h2>
        )}
        ...
        {info.kind === 'before' && (
          <div>
            <h2 className="text-[24px] sm:text-[28px] font-medium text-fg leading-tight">Awaiting start</h2>
            <p className="text-fg-variant text-sm mt-1">Program begins {state.startDate}.</p>
          </div>
        )}
        {info.kind === 'complete' && (
          <div>
            <h2 className="text-[24px] sm:text-[28px] font-medium text-fg leading-tight">Program complete</h2>
            <p className="text-fg-variant text-sm mt-1">All 60 days done.</p>
          </div>
        )}

        {/* Sub note for rest types */}
        {info.kind === 'rest' && (
          <p className="text-fg-variant text-[14px] -mt-2">A planned rest day in the program.</p>
        )}
        {info.kind === 'extra-rest' && (
          <p className="text-fg-variant text-[14px] -mt-2">The remaining program shifts forward by one day.</p>
        )}

        {/* Video link */}
        {info.kind === 'workout' && (
          <a
            href={`https://youtu.be/${info.entry.v}`}
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 text-[13px] text-primary hover:text-primary-hover"
          >
            youtu.be/{info.entry.v}
        ...
        {(canDo || isExtraRest) && (
          <div className="flex flex-wrap items-center gap-2 pt-2">
            {canDo && !isDone && (
              <FilledButton onClick={onComplete} icon={<CheckIcon />}>Mark complete</FilledButton>
            )}
            {isDone && (
              <TonalButton onClick={onComplete}>Unmark complete</TonalButton>
            )}
            {!isExtraRest && info.kind !== 'before' && info.kind !== 'complete' && (
              <TonalButton onClick={onRest}>Mark as extra rest</TonalButton>
            )}
            {isExtraRest && (
              <TonalButton onClick={onRest}>Remove extra rest</TonalButton>
            )}
          </div>
        )}
      </div>
...
function CalendarCard({ viewMonth, setViewMonth, state, program, today, selectedDate, onSelectDay }) {
  const { y, m } = viewMonth;
  ...
  return (
    <section className="bg-surface-1 rounded-xl p-4 sm:p-5">
...
function DayCell({ date, dayNum, state, program, isToday, isPast, isSelected, onSelect }) {
  const info = dayInfo(date, state, program);
  const isDone = state.completed.includes(date);
  const cat = info.kind === 'workout' ? categoryOf(info.entry.n) : null;
